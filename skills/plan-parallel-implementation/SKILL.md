...
- **Definition of done must always include a self-authored code-flow review guide.** Once implementation is complete, the agent produces a short guide for whoever reviews its diff, structured as a walkthrough of the code's execution path rather than a prioritized file list. It must:
  1. Open with one line: path + branch.
  2. Add a section **How to review this (code flow)** that names the single primary entry point for the track's change (verified from the actual code, not guessed), then walks the reviewer through the happy-path call sequence in execution order — "Start at `X` — this is the thing that … It calls `Y` — open that next; what happens inside is … Then `Z` — this is where …" — giving, at each hop, the file path, the key function, and one sentence of *why you're there in the flow*, not a risk score.
  3. Keep mutation/external-call/LLM steps in place in the sequence where they actually execute — never pulled to the front as a "high risk" tier separate from the flow.
  4. Call out points where the flow branches (error paths, retries, alternate modes, idempotency/re-run handling) and name the function that owns each branch.
  5. End with what can be skimmed once the flow is clear (types, barrels, generated/mechanical/renamed files).
  Sized to how large the track's own diff actually turned out to be (near-nothing for a small track, a real walkthrough for a large one). Tone is a senior engineer walking a colleague through the code — imperative navigation ("open this, then follow into…") — never a prioritized checklist of scary files. The agent that wrote the code is best positioned to describe this flow — don't leave it for a downstream review skill to reconstruct after the fact from a cold read of the diff.
...
Each task entry should be short: point at its `plans/track-*.md` (or `00-foundation.md`) file rather than restating its contents, and instruct the agent to read that file, `CLAUDE.md`/`AGENTS.md` (if present), and the referenced doc sections before writing any code. Restate the code-flow review guide requirement from Step 4 explicitly in the prompt too — don't rely on the agent finding it buried in the plan file alone, since it's easy to skip once the "real" implementation work feels done.
